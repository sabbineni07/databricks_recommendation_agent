...
class LocalDataCollector:
    ...
    def collect_resource_utilization(
        ...
        """
        logger.info("collecting_resource_utilization_from_csv", start_date=start_date, end_date=end_date)
        ...
        try:
            ...
            for metric in metrics:
                ...
                utilization_by_job[job_id]['avg_cpu_utilization_pct'].append(metric.avg_cpu_utilization_pct)
                utilization_by_job[job_id]['avg_memory_utilization_pct'].append(metric.avg_memory_utilization_pct)
                utilization_by_job[job_id]['peak_cpu_utilization_pct'].append(metric.peak_cpu_utilization_pct)
                utilization_by_job[job_id]['peak_memory_utilization_pct'].append(metric.peak_memory_utilization_pct)
            ...
            for job_id, data in utilization_by_job.items():
                result.append({
                    'job_id': job_id,
                    'avg_cpu_utilization_pct': sum(data['avg_cpu_utilization_pct']) / len(data['avg_cpu_utilization_pct']),
                    'avg_memory_utilization_pct': sum(data['avg_memory_utilization_pct']) / len(data['avg_memory_utilization_pct']),
                    ...
                    'avg_nodes_consumed': sum(data['avg_nodes_consumed']) / len(data['avg_nodes_consumed']),
    ...
    def collect_cost_data(
        ...
        try:
            ...
            for job_id, data in cost_by_job.items():
                avg_cost_per_hour = sum(data['cost_per_hour_usd']) / len(data['cost_per_hour_usd']) if data['cost_per_hour_usd'] else 0.0
                avg_cost_per_run = data['total_cost_usd'] / data['total_runs'] if data['total_runs'] > 0 else 0.0
