.PHONY: help install setup up down restart logs shell test clean migrate init-db backup-db restore-db lint format run-api run-test test-local build rebuild psql pgadmin
...
help: ## Display this help message
	@echo "$(BLUE)Databricks Recommendation Agent - Makefile Commands$(NC)"
	@echo ""
...
health: ## Check API health
	...
	@curl -s http://localhost:8000/api/health | python -m json.tool || echo "$(YELLOW)API not running$(NC)"
...
info: ## Show project information
	...
	@echo ""
	@echo "$(GREEN)Project:$(NC) Databricks Recommendation Agent"
