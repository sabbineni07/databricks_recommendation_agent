...
class CostUsageLog(Base):
    """Model for tracking token usage and costs per LLM call."""
...
class DailyCostSummary(Base):
    """Model for daily aggregated cost summaries."""
...
class RecommendationHistory(Base):
    ...
    workspace_id = Column(String(255), nullable=True)
    timestamp = Column(DateTime(timezone=True), default=func.now(), index=True)
    recommendation = Column(JSONB, nullable=False)  # PostgreSQL JSONB for better performance
