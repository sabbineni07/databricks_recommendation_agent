...
COST_RECOMMENDATION_KEYS = ("node_family", "vcpus", "min_workers", "max_workers", "auto_termination_minutes", "rationale")
...
- rationale: string (2-4 sentences: cite metrics and briefly address VM family, CPU/memory fit, disk I/O if relevant, and version if relevant)"""),
            ("human", """## Input: Current configuration
{current_config}
...
Using only the inputs above, output a single JSON object with keys node_family, vcpus, min_workers, max_workers, auto_termination_minutes, rationale. No other text, no markdown.""")
    ...
    def optimize(self, current_config: dict, job_cluster_metrics: dict, budget_constraints: dict, pattern_analysis: str = "") -> dict:
        ...
        try:
            ...
            avg_nodes = (job_cluster_metrics or {}).get("avg_nodes_consumed") or (job_cluster_metrics or {}).get("p95_nodes_consumed")
            max_workers = 8
